...
Fake = pd.read_csv(r"/content/drive/MyDrive/Fake.csv", encoding='latin-1', on_bad_lines='skip')
Fake
Real = pd.read_csv(r"/content/drive/MyDrive/True.csv", encoding='latin-1', on_bad_lines='skip')
Real
# load our saved models using pickle 

tree_path = "/content/svm_model.pkl"
...
def main():
  ...
  st.dataframe(Fake['text'].sample(1), hide_index = True)
  
  st.write('''#### Real News Article''')
  st.write('''Click the box below and copy/paste.''')
  st.dataframe(Real['text'].sample(1), hide_index = True)
